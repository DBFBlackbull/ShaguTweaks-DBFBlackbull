...
local function AddLockPickingSkill(frame, itemLink)
    ...
    if requirePickPocketSkill[itemID] then
        lineString = string.format(T["Requires Lockpicking (%d)"], requirePickPocketSkill[itemID])
    end

    if canOpenPickPocketSkill[itemID] then
        lineString = string.format(T["Opens locks up to (%d)"], canOpenPickPocketSkill[itemID])
    end
